(function () {
  ...
  const isCollectionPage =
    window.location && window.location.pathname.includes("/collections/");
  const isCartPage =
  ...
  function appendAnnoucementBarCss(config, animationDuration) {
    ...
    const css = `
          .essential_annoucement_bar_wrapper_${config.id} {
            ...
            border: ${styleConfig.borderColor} solid ${
      styleConfig.borderSize
    }px;
            ${
  ...
  function addAnnouncementBar() {
    ...
    const canShowOnCollection = (config) => {
      if (config.showOnCollections && config.showOnCollections.length !== 0) {
        ...
        const showOnCollection = config.showOnCollections.find((collection) =>
    ...
    const canShowInCountry = (
      ...
      if (
        ...
        !clientCountry &&
        (Shopify.shop === "06567d-08.myshopify.com" ||
          Shopify.shop === "zydrunas-test-store2.myshopify.com")
      ) {
        return false;
      }
      if (
